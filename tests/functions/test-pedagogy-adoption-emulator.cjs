...
(async () => {
  try {
    ...
    const sourceSnapshot = [{ id: 'synthetic-source', version: 1, subjectId: 'math', classSubjectId: 'math', curriculumUnitId: null, lessonTitle: 'Synthetic fractions', objective: null, pedagogicalContent: '1/2 = 2/4 by doubling numerator and denominator; 2/6 = 1/3; 3 + 4 = 7. Only these examples; no multiplication of two fractions.', teachingConfirmationId: 'synthetic-confirmation', teachingStatus: 'taught', effectiveTeachingDate: '2026-09-08' }];
    const qualityAssessment = { generatorProvider: 'openai', generationVersion: 1, itemCount: 1, sourceSnapshot, sourceChecksum: sourceChecksum(sourceSnapshot) };
    ...
    const sourceFields = { subjectId: 'math', classSubjectId: 'math', sourceLessonPreparationIds: ['synthetic-source'], sourceCurriculumUnitIds: [] };
    await qualityRef.update({ ...sourceFields, ...bindTaughtSnapshot(sourceFields, sourceSnapshot) });
    ...
    await checkQuality();
    await qualityRef.update({ correctionGuide: '1/2 × 2/2 = 2/4' });
    await assert.rejects(checkQuality(), error => error.code === 'failed-precondition');
    await qualityRef.update({ correctionGuide: 'Numerator 1 × 2 = 2; denominator 2 × 2 = 4.' });
    await checkQuality();
    await qualityRef.update({ sourceSnapshotChecksum: 'stale' });
    await assert.rejects(checkQuality(), error => error.code === 'failed-precondition');
