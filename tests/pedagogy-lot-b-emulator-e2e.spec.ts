...
test.describe('Lot B — préparations de cours', () => {
  ...
  test('dérive, dépose, analyse, corrige, valide et nettoie sans régression Lot A', async ({ page }) => {
    ...
    try {
      ...
      await test.step('Open and export an original resource', async () => {
      ...
      await page.getByText('Provenance, couverture et revue pédagogique', { exact: true }).click();
      ...
      console.log('Synthetic resource checkpoint: requesting download');
      ...
      console.log('Synthetic resource checkpoint: download event received');
      expect(draftDownload.suggestedFilename()).toBe('original-pre-en-v1.txt');
      const stream = await draftDownload.createReadStream();
      console.log('Synthetic resource checkpoint: stream opened');
      ...
      console.log('Synthetic resource checkpoint: stream completed');
      ...
      console.log('Synthetic resource checkpoint: file cleaned');
      }, { timeout: 25_000 });
