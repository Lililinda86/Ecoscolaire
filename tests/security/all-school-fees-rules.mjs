...
try {
  await env.withSecurityRulesDisabled(async c => {
    ...
    for (const collection of ['studentFinancialObligations', 'studentFeeAssignments', 'studentTransportPlans']) {
  ...
  for (const role of ['owner', 'director', 'secretary', 'accountant']) {
    ...
    await assertFails(updateDoc(doc(db, 'schools', 'fee-school'), { classFees: { CP: { t1: 1 } } }));
    await assertFails(updateDoc(doc(db, 'schools', 'fee-school'), { globalFees: { feeT1: 1 } }));
    await assertFails(updateDoc(doc(db, 'schools', 'fee-school'), { financialTariffVersion: 'forged' }));
    await assertFails(setDoc(doc(db, 'schools', 'fee-school', 'financialTariffVersions', 'forged'), { next: { amount: 1 } }));
  }
