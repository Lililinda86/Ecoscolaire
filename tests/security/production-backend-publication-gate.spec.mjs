...
// Deployment configuration must route through the tested gate even when dashboard settings override npm defaults.
test('Vercel repository configuration invokes the gated build',async()=>{
 const fs=await import('node:fs');
 assert.equal(JSON.parse(fs.readFileSync('vercel.json','utf8')).buildCommand,'npm run vercel-build');
});
