...
describe('bounded source-watch policy', () => {
  it.each(['https://www.minesec.gov.cm/web/index.php/fr/systeme-educatif/progammes-officiels', 'https://www.minedub.cm/', 'https://camgceb.org/downloads/'])('allows an explicitly listed public host without claiming authenticity: %s', url => {
  ...
  it('restricts the original controlled-change fixture to explicit synthetic Staging', () => {
    expect(() => sourceWatchUrl('https://camgceb.org.attacker.invalid/downloads/', false)).toThrow('SOURCE_HOST_NOT_AUTHORIZED');
    expect(() => sourceWatchUrl('https://camgceb.org/downloads/?token=private', false)).toThrow('SOURCE_URL_UNSAFE');
