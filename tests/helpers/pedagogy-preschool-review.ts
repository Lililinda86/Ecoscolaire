...
export async function verifyPreschoolReview(page: Page, db: Firestore, f: { schoolId: string; teacherId: string; pupilIds: string[] }) {
  ...
  await page.getByLabel('Activité enseignée', { exact: true }).selectOption(`${f.schoolId}-prep-math`);
  await page.getByLabel('Objectif observable, extrait exact du contenu confirmé').fill('Synthetic objective');
  await page.getByLabel('Enseignant déclarant', { exact: true }).selectOption(f.teacherId);
  ...
  await page.getByLabel('Preuve initiale', { exact: true }).selectOption('observation:' + observation.id);
  ...
  for (const button of ['Consigner l’accord enseignant', 'Consigner la réalisation']) {
    ...
    await support.getByLabel('Enseignant déclarant', { exact: true }).selectOption(f.teacherId);
