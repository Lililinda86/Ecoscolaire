...
async function seedResultsFixture(db, prefix, reviewCase = { name: 'CE1', section: 'francophone', stage: 'primary' }) {
  ...
  put('classes', f.classId, { schoolId: f.schoolId, name: reviewCase.name, type: reviewCase.section, section: reviewCase.section, cycle: reviewCase.stage === 'preschool' ? 'nursery' : reviewCase.stage, isActive: true, status: 'active', isTestFixture: true });
  ...
  put('curriculumPrograms', prefix + '-curriculum', { title: 'SYNTHETIC review curriculum — not official', countryCode: 'CM', section: reviewCase.section, cycle: reviewCase.stage, version: 'synthetic-v1', status: 'published', sourceType: 'mock', schoolId: f.schoolId, isTestFixture: true });
  put('teachingPlans', prefix + '-plan', { schoolId: f.schoolId, academicYearId: f.academicYearId, classId: f.classId, weekId: f.weekId, weekNumber: 1, weekStartDate: '2026-08-31', weekEndDate: '2026-09-04', status: 'teacher_validated', version: 1, isTestFixture: true, teacherValidationNote: 'Synthetic stored fixture only; no real teacher declaration' });
  ...
  for (const [index, suffix, title] of [[1, 'math', 'Mathematics'], [2, 'english', 'English']]) {
    ...
    const unitId = prefix + '-unit-' + suffix, planItemId = prefix + '-plan-item-' + suffix, templateId = prefix + '-template-' + suffix;
    put('curriculumUnits', unitId, { programId: prefix + '-curriculum', schoolId: f.schoolId, catalogLevelId: 'synthetic-review-level', subjectId, title, objective: 'Synthetic objective', competency: 'Synthetic competency: identify and explain', sequence: index, status: 'published', sourceType: 'mock', isTestFixture: true });
    put('teachingPlanItems', planItemId, { schoolId: f.schoolId, academicYearId: f.academicYearId, classId: f.classId, planId: prefix + '-plan', subjectId, subjectName: title, teacherStaffId: f.teacherId, curriculumUnitId: unitId, lessonTitle: title, objective: 'Synthetic objective', dayIndex: 2, slotIndex: index, status: 'teacher_validated', isTestFixture: true });
    put('lessonPreparationTemplates', templateId, { schoolId: f.schoolId, academicYearId: f.academicYearId, classId: f.classId, subjectId, planId: prefix + '-plan', planItemId, lessonTitle: title, objective: 'Synthetic objective', isTestFixture: true });
    put('lessonPreparations', prepId, { schoolId: f.schoolId, academicYearId: f.academicYearId, classId: f.classId, weekId: f.weekId, weekStartDate: '2026-08-31', weekEndDate: '2026-09-04', subjectId, subjectName: title, lessonTitle: title, teacherStaffId: f.teacherId, planId: prefix + '-plan', planItemId, templateId, curriculumUnitId: unitId, version: 1, status: 'validated', reviewData, currentUploadId, isTestFixture: true,
  ...
  if (reviewCase.stage !== 'preschool') put('weeklyAssessments', f.assessmentId, { schoolId: f.schoolId, academicYearId: f.academicYearId, classId: f.classId, className: reviewCase.name, weekId: f.weekId, weekStartDate: '2026-08-31', weekEndDate: '2026-09-04', fridayDate: '2026-09-04', title: 'Synthetic weekly assessment', status: 'teacher_validated', generationStatus: 'succeeded', generationVersion: 1, contentRevision: 0, teacherValidated: true, teacherValidations: decisions, sourceChecksum: 'synthetic-source-checksum', sourcePreparationVersions: versions, sourcePreparationIds: Object.keys(versions), expectedPreparationCount: 2, validatedPreparationCount: 2, itemCount: 2, totalPoints: 20, durationMinutes: 60, sections, coveredSubjects: sections.map(item => ({ id: item.subjectId, name: item.title })), missingSubjects: [], policySnapshot: { version: 1, assessmentMode: 'numeric', totalPoints: 20, stage: reviewCase.stage } });
  await batch.commit();
