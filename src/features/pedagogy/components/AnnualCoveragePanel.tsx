...
export function AnnualCoveragePanel({levelIds}:{levelIds:string[]}) {
 ...
 const labels={COMPLETE:'Complet',PARTIAL:'Partiel',SOURCE_MISSING:'Source exploitable manquante',STRUCTURING_PENDING:'À structurer',NOT_APPLICABLE:'Non applicable'};
 return <section className="pedagogy-card" aria-label="Couverture annuelle documentaire">
  ...
  {rows.map(row=><article key={row.id} className="pedagogy-list-row"><div>
   ...
   {earlyYearsAnnualPlans.filter(p=>row.structuredContent.includes(p.id)).map(p=><div key={p.id}><h4>Progression Ecoscolaire proposée — programme local ITALO</h4>{p.phases.map(phase=><p key={phase.label}><strong>{phase.label}</strong> : {phase.action}</p>)}<p>{p.adaptation}</p><p>{p.safety}</p></div>)}
   </details>
  </div></article>)}
  </details>
 </section>;
