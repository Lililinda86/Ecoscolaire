...
const scopeLabel=(s:string)=>s.startsWith('EXACT_CLASS_COLUMN_')?'Synthèse de la colonne de classe ; réserve documentaire non levée.':s.startsWith('LEVEL_')?'Objectifs communs aux deux classes du niveau ; tableaux propres à chaque classe à compléter.':s==='LOCATED_MODULE_NOT_COMPLETE_LESSON'?'Module localisé ; leçons détaillées à compléter.':s==='SHARED_INTRODUCTION_NOT_CLASS_DETAIL'?'Introduction commune ; détail par classe à compléter.':s;
export function AnnualCoveragePanel({levelIds,actual}:{levelIds:string[];actual?:ReturnType<typeof actualAnnualReadiness>}) {
 ...
 return <section className="pedagogy-card" aria-label="Couverture annuelle documentaire">
  ...
  <p>Contenu annuel structuré : {totals.structuredAnnual}/{totals.expectedDocumentaryScopes} ({totals.structuredAnnualCoveragePercentage.toFixed(2)} %). Les réserves documentaires et la configuration locale restent à vérifier.</p>
  ...
  {rows.map(row=><article key={row.id} className="pedagogy-list-row"><div>
   <h3>{levelLabel(row.catalogLevelId)} · {row.subjectName}</h3><p>Année · {labels[row.coverageStatus]}</p><p>{row.missingReason}</p>{row.partialCause&&<p>Cause : {row.partialCause}</p>}
