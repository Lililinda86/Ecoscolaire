...
function WatchEditor({ schoolId, slot, watch, canEdit, onDirty, onSaved }: {
  ...
  const changed = () => onDirty(true);
  const save = async () => {
    if (busy || !canEdit || uncertain) return;
  ...
  return <div>
    ...
    {canEdit && <div className="pedagogy-actions"><button className="pedagogy-button" disabled={busy || uncertain || !title.trim() || !url.trim()} onClick={() => void save()}>Enregistrer la veille</button><button className="pedagogy-button pedagogy-button--secondary" disabled={busy} onClick={() => void reload()}>Recharger la configuration enregistrée</button></div>}
