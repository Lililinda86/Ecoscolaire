...
export function PrimarySubjectSteps({ approvedLevels, levelsAvailable, levelsLoading }: { approvedLevels: string[]; levelsAvailable: number; levelsLoading: boolean }) {
  ...
  return <div className="primary-owner-review">
    ...
    <section aria-label="Étape 2 — Correspondances de matières sûres"><h3>Étape 2 — CONFIRMER LES MATIÈRES ÉVIDENTES</h3>
      ...
      {['fr-', 'en-'].map(prefix => <section key={prefix} aria-label={prefix === 'fr-' ? 'Matières sûres primaire FR' : 'Matières sûres primaire EN'}><h4>{prefix === 'fr-' ? 'Primaire FR' : 'Primaire EN'}</h4>{scope.rows.filter(row => row.catalogLevelId.startsWith(prefix)).map(row => {
        ...
        return <article key={row.classId} aria-label={'Correspondances sûres — ' + row.className}><h4>{row.className}</h4><p><a href={row.source.sourceUrl} target="_blank" rel="noopener noreferrer">Source MINEDUB</a> · {mappings.length} correspondances sûres</p>
          ...
          <p><a href={'#primary-cases-' + row.classId} onClick={event => { event.preventDefault(); document.getElementById('primary-cases-' + row.classId)?.scrollIntoView({ block: 'start' }); }}>Voir les {row.mappings.filter(m => m.status === 'AMBIGUOUS').length} cas à examiner — {row.className}</a></p>
    ...
    <section aria-label="Étape 3 — Décisions nécessaires"><h3>Étape 3 — RÉSOUDRE LES CAS À EXAMINER</h3><p>{ambiguous.length} cas initiaux : {clarified} avec une relation de composante documentée, {ambiguous.length - clarified} dont le périmètre local reste à préciser. Les {ambiguous.length} choix d’utilisation locale restent humains. « À revoir plus tard » reste en attente. Aucune option n’est préchoisie.</p>
      {ambiguous.map(({ row, mapping }) => {
        ...
        return <article key={key} id={row.mappings.find(m => m.status === 'AMBIGUOUS') === mapping ? 'primary-cases-' + row.classId : undefined} data-testid="primary-ambiguous-mapping" aria-label={'Décision nécessaire — ' + row.className + ' — ' + mapping.officialSubject}>
