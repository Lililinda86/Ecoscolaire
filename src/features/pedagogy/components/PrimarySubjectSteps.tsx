...
export function PrimarySubjectSteps({ approvedLevels, levelsAvailable, levelsLoading }: { approvedLevels: string[]; levelsAvailable: number; levelsLoading: boolean }) {
  ...
  const localChoice = (row: SubjectReviewRow, mapping: SubjectMatch) => (scope.delegatedDecisions || []).find(d => d.schoolId === scope.schoolId && d.academicYearId === scope.yearId && d.decisionOrigin === 'OWNER_DELEGATED_VALIDATION' && d.evidence?.localSubjects?.length === mapping.candidates.length && mapping.candidates.every(c => d.evidence?.localSubjects?.some(s => s.id === c.id && s.name === c.name)) && d.scope === 'ITALO_PEDAGOGICAL_CHOICE' && d.classId === row.classId && d.officialSubject === mapping.officialSubject && d.sourceVersion === row.source.sourceVersion && d.decision === 'KEEP_LOCAL_SUBJECTS_DISTINCT');
  const locallyConfigured = toReview.filter(({ row, mapping }) => localChoice(row, mapping)).length;
  ...
  return <div className="primary-owner-review">
    ...
    <section aria-label="Étape 3 — Décisions nécessaires"><h3>Étape 3 — RÉSOUDRE LES CAS À EXAMINER</h3><p>{ambiguous.length} cas initiaux : {clarified} avec une relation de composante documentée. {documentarilyResolved.length} dossiers documentaires enregistrés sans question locale supplémentaire ; {toReview.length - locallyConfigured} situations sans configuration locale enregistrée ; {locallyConfigured} configurations distinctes validées par délégation, périmètre documentaire restant à préciser. « À revoir plus tard » reste en attente. Aucune option n’est préchoisie.</p>
      ...
      {toReview.map(({ row, mapping }) => {
        ...
        return <article key={key} id={toReview.find(item => item.row.classId === row.classId)?.mapping === mapping ? 'primary-cases-' + row.classId : undefined} data-testid="primary-ambiguous-mapping" aria-label={'Décision nécessaire — ' + row.className + ' — ' + mapping.officialSubject}>
          ...
          <p>Décision actuelle : {existing ? decisionLabels[existing.decision] : localChoice(row, mapping) ? 'Matières conservées distinctes — validation déléguée' : 'En attente'}</p>{localChoice(row, mapping) && <p>{localChoice(row, mapping)!.reason} Aucun nouveau clic nécessaire pour conserver cette organisation.</p>}{existing?.decisionNote && <p>{existing.decisionNote}</p>}
