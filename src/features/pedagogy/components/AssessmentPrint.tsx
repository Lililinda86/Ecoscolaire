...
export function AssessmentPrint({ school, assessment, items, mode, sourceChanged, language, academicYearLabel }: {
  ...
  return <section id="weekly-assessment-print" className="pedagogy-a4 assessment-paper" lang={language}>
    ...
    {items.map(item => <article className="assessment-question" key={item.id}><h3>{item.order}. {readableAssessmentText(item.questionText)} <small>({item.points} pt{item.points > 1 ? 's' : ''})</small></h3><p>{readableAssessmentText(item.instructions)}</p>{item.questionType === 'multiple_choice' && !!item.choices?.length && <ol type="A">{item.choices.map((choice, index) => <li key={index}>{readableAssessmentText(choice)}</li>)}</ol>}{correction ? <><strong>{en ? 'Expected answer' : 'Réponse attendue'}</strong>{item.questionType === 'multiple_choice' && item.correctAnswer && <p>{readableAssessmentText(item.correctAnswer)}</p>}<p>{readableAssessmentText(item.expectedAnswer)}</p><strong>{en ? 'Marking guidance' : 'Consignes de correction'}</strong><p>{readableAssessmentText(item.correctionGuide)}</p></> : <div className="assessment-answer-lines" />}</article>)}
