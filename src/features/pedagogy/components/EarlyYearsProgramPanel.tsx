...
import type { SchoolCurriculumAdoption } from '../types';

export function EarlyYearsProgramPanel({ weeks = [], adoptions = [], yearId }: { weeks?: Array<{ id: string; weekStartDate: string }>; adoptions?: SchoolCurriculumAdoption[]; yearId?: string }) {
  ...
  const adoption = adoptions.find(a => a.schoolId === currentSchool?.id && a.academicYearId === yearId && a.catalogLevelId === selected?.catalogLevelId && a.status === 'active' && a.programVersion === '1.0' && a.curriculumProgramId === 'italo-early-years-v1-' + selected?.catalogLevelId && a.programKind === 'ITALO_EARLY_YEARS_PROGRAM' && a.validationDecision === 'ITALO_LOCAL_PROGRAM_VALIDATED');
  const template = (activity: typeof activities[number]) => earlyYearsTemplate(activity).map(([label, value], i) => [label, i === 0 && adoption ? 'ITALO_EARLY_YEARS_PROGRAM — LOCAL_VALIDATED · validation déléguée' : value]);
  const printActivity = activities.find(a => a.id === printId);
  const exportTemplate = (activity: typeof activities[number]) => {
    const text = template(activity).map(([label, value]) => label + '\n' + value).join('\n\n');
  ...
  return <section className="pedagogy-card" data-testid="early-years-program">
    <div className="no-print">
      ...
      <p><strong>ITALO_EARLY_YEARS_PROGRAM · {adoption ? 'programme local validé par délégation' : 'proposition locale à relire'}</strong></p>
      ...
      <label>Niveau préscolaire ITALO<select aria-label="Niveau préscolaire ITALO" value={selected ? levelId : ''} onChange={e => { setLevelId(e.target.value); setWorked([]); setPrintId(''); }}><option value="">Choisir un niveau configuré</option>{classes.map(c => <option key={c.id} value={c.catalogLevelId}>{c.name}</option>)}</select></label>
      {!classes.length && <p>Aucun niveau préscolaire actif dans cet établissement. Aucune classe créée automatiquement.</p>}
      {selected && <>
        {adoption && <p role="status">Validation locale enregistrée pour cette école et cette année. Autorisée par le propriétaire, enregistrée par {adoption.decisionRecordedBy}. L’adaptation de la séance et la réalisation restent distinctes.</p>}
        ...
        {activities.map(activity => <details key={activity.id} className="pedagogy-template-section" lang={activity.language}>
          ...
          <details><summary>Modèle de préparation préscolaire complet</summary>{template(activity).map(([label, value]) => <p key={label}><strong>{label}</strong><br />{value}</p>)}</details>
          ...
          <details><summary>Voir les détails de provenance</summary><p>Version ITALO {activity.version} · {activity.verificationStatus} · {adoption ? 'LOCAL_VALIDATED' : activity.humanReviewStatus}</p><p style={{ overflowWrap: 'anywhere' }}>SHA-256 source : {activity.domainReference.checksum}</p><p>Équivalence de niveau officiel : non établie. Durée : à définir par l’enseignant. Aucun horaire, coefficient ou enseignant affecté.</p></details>
        </details>)}
    ...
    {printActivity && <section className="pedagogy-a4 assessment-paper" lang={printActivity.language}>
      ...
      {template(printActivity).map(([label, value]) => <article className="assessment-question" key={label}><h3>{label}</h3><p>{value}</p></article>)}
    </section>}
  </section>;
}
