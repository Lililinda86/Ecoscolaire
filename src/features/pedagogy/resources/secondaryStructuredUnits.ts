 ...
 * Unknown detailed outcomes, activities and assessment remain null. */
export interface SecondaryStructuredUnit {
  id: string; catalogLevelId: string; level: string; language: 'fr' | 'en'; subject: string;
  title: string; objective: string; competency: string | null; activity: string | null;
...
const units = (file: string, level: string, catalogLevelId: string, language: 'fr' | 'en', rows: Row[], aimIndices: number[]) => rows.map(([title, sourcePage, documentaryHours], i): SecondaryStructuredUnit => ({
  ...
  competency: null, activity: null, assessment: null, sourceDocumentId: 'minesec-' + file,
...
export const secondaryStructuredUnits: SecondaryStructuredUnit[] = [
  ...
  ...units('8', 'Form 1', 'en-secondary-form1', 'en', [['Numbers and operations',20,30],['Plane shapes',20,45],['Solid shapes',20,15],['Data and probability',20,10]], [0,1,2,3]),
