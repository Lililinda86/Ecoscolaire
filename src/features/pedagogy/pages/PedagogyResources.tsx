...
export default function PedagogyResources() {
  ...
  return <div className="pedagogy-page">
    ...
    <section className="pedagogy-card">
      <div className="pedagogy-filters"><label>Langue<select value={language} onChange={event => setLanguage(event.target.value)}><option value="">Toutes</option><option value="fr">Français</option><option value="en">English</option></select></label><label>Cycle<select value={cycle} onChange={event => setCycle(event.target.value)}><option value="">Tous</option>{Object.entries(cycleLabels).map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select></label><label>Recherche<input value={search} onChange={event => setSearch(event.target.value)} placeholder="Titre ou objectif" /></label></div>
