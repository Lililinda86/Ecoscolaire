  ...
  return <main className="pedagogy-page">
    ...
    {assessment && <section className="pedagogy-card no-print">
      ...
      <div className="assessment-editor">{items.map((item, index) => <article key={item.id}>
        ...
        {item.questionType === 'multiple_choice' && <><label>Choix QCM (un par ligne, 2 à 5)<textarea value={(item.choices || []).join('\n')} onChange={event => setItems(current => current.map(row => row.id === item.id ? { ...row, choices: event.target.value.split('\n'), correctAnswer: row.correctAnswer ?? '' } : row))} /></label><label>Bonne réponse (texte exact d’un choix)<input value={item.correctAnswer || ''} onChange={event => setItems(current => current.map(row => row.id === item.id ? { ...row, choices: row.choices || [], correctAnswer: event.target.value } : row))} /></label></>}
      ...
      <details style={{ overflowWrap: 'anywhere' }}><summary>Sources enseignées — périmètre de revue</summary><p>Comparer toutes les questions, réponses et explications à ce snapshot. Une absence d’alerte n’est pas une preuve de fidélité. Le curriculum et les ressources ne peuvent pas élargir les notions évaluées.</p>
        {(assessment.sourceSnapshot || []).map((source, index) => <div key={index}><h3>{String(source.lessonTitle || source.id || 'Source')}</h3><p>Préparation {String(source.id)} · version {String(source.version)} · déclaration {String(source.teachingConfirmationId || 'absente')}</p><p style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{String(source.pedagogicalContent || '')}</p></div>)}
        {items.map(item => <p key={item.id}>Question {item.order} · sources : {item.sourceLessonPreparationIds.join(', ')} · {item.coverageIssues?.length ? item.coverageIssues.join(', ') : 'Revue humaine de fidélité requise'}</p>)}
      </details>
      {assessment.status === 'needs_review' && <div className="pedagogy-actions">
        ...
        <label><input type="checkbox" checked={confirmedDeclaration} onChange={event => { setDeclarationReceived(event.target.checked); setDeclarationScope(reviewScopeKey); }} /> Accord enseignant reçu pour cette version et ces matières — fidélité des questions, réponses et corrigés aux seules portions enseignées vérifiée par l’enseignant</label>
