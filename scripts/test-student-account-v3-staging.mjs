...
const main = async () => {
  ...
  try {
    ...
    for (const width of [360, 768, 1440]) {
      ...
      const studentSearch = page.locator('#student-account-search');
      await studentSearch.fill('Élève Compte');
      assert.equal(await page.getByTestId('cash-payment-student').locator(`option[value="${studentId}"]`).count(), 1);
      await studentSearch.fill(`V3-${suffix}`.slice(0, 80));
      assert.equal(await page.getByTestId('cash-payment-student').locator(`option[value="${studentId}"]`).count(), 1);
      await studentSearch.fill('introuvable');
      assert.equal(await page.getByTestId('cash-payment-student').locator(`option[value="${studentId}"]`).count(), 0);
    ...
    await page.getByTestId('cash-payment-student').selectOption(studentId);
    await page.getByRole('heading', { name: 'Frais applicables' }).waitFor({ state: 'visible', timeout: 30_000 });
    await page.getByLabel("Montant reçu pour Frais d'inscription").fill('1000');
    await page.getByLabel('Montant reçu pour Scolarité T2').fill('2000');
    const basket = page.locator('.collection-basket');
    await basket.getByText("Frais d'inscription", { exact: true }).waitFor({ state: 'visible' });
    await basket.getByText('Scolarité T2', { exact: true }).waitFor({ state: 'visible' });
    const basketSubmit = page.getByTestId('cash-payment-submit');
    assert.equal(await basketSubmit.isDisabled(), false);
    assert.match((await basketSubmit.textContent()) || '', /ENCAISSER/);
    await basketSubmit.click();
    await page.getByRole('heading', { name: 'Encaissement validé' }).waitFor({ state: 'visible', timeout: 30_000 });
    const uiReceiptNumber = (await page.locator('.student-account-success > p strong').textContent())?.trim() || '';
    assert.match(uiReceiptNumber, /^REC-/);
    ...
    await page.getByRole('button', { name: 'Nouvel encaissement', exact: true }).click();
    await page.getByRole('button', { name: 'Annuler', exact: true }).click();
    await page.getByRole('button', { name: 'Reçus', exact: true }).click();
    await page.getByText(receipt.receiptNumber, { exact: true }).waitFor({ state: 'visible', timeout: 30_000 });
    const receiptRow = page.getByTestId(`receipt-row-${first.receiptId}`);
    await receiptRow.waitFor({ state: 'visible' });
    await receiptRow.getByTestId(`receipt-detail-toggle-${first.receiptId}`).click();
    await page.getByTestId(`receipt-detail-${first.receiptId}`).waitFor({ state: 'visible' });
    const historyMetrics = await page.getByTestId('receipt-history-scroll').evaluate((element) => ({ documentWidth: document.documentElement.scrollWidth, viewportWidth: document.documentElement.clientWidth, clientWidth: element.clientWidth }));
    assert.ok(historyMetrics.documentWidth <= historyMetrics.viewportWidth + 1 && historyMetrics.clientWidth <= 1440);
    const historyPrint = receiptRow.getByRole('button', { name: 'Imprimer', exact: true });
    await historyPrint.scrollIntoViewIfNeeded();
    const historyPrintBox = await historyPrint.boundingBox();
    assert.ok(historyPrintBox && historyPrintBox.height >= 40 && historyPrintBox.x + historyPrintBox.width <= 1441);
    await context.close();
