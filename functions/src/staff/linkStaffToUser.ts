...
export const linkStaffToUser = functions.https.onCall(async (data, context) => {
  ...
  try {
    return await db.runTransaction(async (transaction) => {
      ...
      if (staff.isActive === false) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Le profil de membre du personnel est inactif.',
      ...
      if (userPointerActive && userPointerSnap.data()?.staffId !== cleanStaffId) {
        throw new functions.https.HttpsError(
          'already-exists',
          'Cet utilisateur est déjà relié à un autre membre du personnel.',
          { businessCode: 'USER_ALREADY_LINKED' }
        );
      }
      ...
      if (staffPointerActive && staffPointerSnap.data()?.userId !== cleanUserId) {
        throw new functions.https.HttpsError(
          'already-exists',
          'Ce membre du personnel est déjà relié à un autre compte utilisateur.',
          { businessCode: 'STAFF_ALREADY_LINKED' }
        );
      }
      ...
      if (userPointerActive && staffPointerActive) {
        ...
        if (uLink.linkId === sLink.linkId && uLink.staffId === cleanStaffId && sLink.userId === cleanUserId) {
          ...
          throw new functions.https.HttpsError(
            'failed-precondition',
            'Incohérence d\'intégrité détectée. L\'utilisateur et le staff sont déjà liés séparément.',
      ...
      if (userPointerActive || staffPointerActive) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Un pointeur orphelin actif a été détecté en base.',
          { businessCode: 'LINK_INTEGRITY_ERROR' }
        );
      }
      ...
      if (userPointerExists && staffPointerExists) {
        ...
        if (uLink.linkId !== sLink.linkId || uLink.staffId !== cleanStaffId || sLink.userId !== cleanUserId) {
          throw new functions.https.HttpsError(
            'failed-precondition',
            'Incohérence d\'intégrité détectée sur les pointeurs inactifs.',
            { businessCode: 'LINK_INTEGRITY_ERROR' }
          );
        }
        ...
        if (!pastLinkSnap.exists || pastLinkSnap.data()?.isActive !== false) {
          throw new functions.https.HttpsError(
            'failed-precondition',
            'Incohérence d\'intégrité avec le document historique inactif.',
            { businessCode: 'LINK_INTEGRITY_ERROR' }
          );
        }
      ...
      if (userPointerExists || staffPointerExists) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Un seul des pointeurs inactifs existe en base.',
          { businessCode: 'LINK_INTEGRITY_ERROR' }
        );
      }
