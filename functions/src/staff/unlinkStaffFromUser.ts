...
export const unlinkStaffFromUser = functions.https.onCall(async (data, context) => {
  ...
  if (typeof staffId !== 'string' || staffId.trim() === '' || staffId.includes('/') || staffId.length > 100) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'staffId doit être une chaîne non vide valide.',
      { businessCode: 'INVALID_ARGUMENT' }
    );
  }
  ...
  if (typeof userId !== 'string' || userId.trim() === '' || userId.includes('/') || userId.length > 100) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'userId doit être une chaîne non vide valide.',
      { businessCode: 'INVALID_ARGUMENT' }
    );
  }
  ...
  if (typeof reason === 'string') {
    ...
    if (cleanReason.length > 500) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Le motif de dissociation est trop long (maximum 500 caractères).',
        { businessCode: 'INVALID_ARGUMENT' }
      );
    }
  }
  ...
  try {
    return await db.runTransaction(async (transaction) => {
      ...
      if (!allowedRoles.includes(operator.role)) {
        throw new functions.https.HttpsError(
          'permission-denied',
          'Rôle non autorisé à gérer les liaisons d\'identité.',
          { businessCode: 'PERMISSION_DENIED' }
        );
      }
      ...
      if (targetUser.schoolId !== cleanSchoolId) {
        throw new functions.https.HttpsError(
          'permission-denied',
          'L\'utilisateur cible n\'appartient pas à la même école.',
      ...
      if (!staffSnap.exists) {
        throw new functions.https.HttpsError(
          'not-found',
          'Profil de membre du personnel introuvable.',
          { businessCode: 'STAFF_NOT_FOUND' }
        );
      }
      ...
      if (staff.schoolId !== cleanSchoolId) {
        throw new functions.https.HttpsError(
          'permission-denied',
          'Le membre du personnel n\'appartient pas à la même école.',
          { businessCode: 'SCHOOL_MISMATCH' }
        );
      }
      ...
      if (!userPointerExists && !staffPointerExists) {
        throw new functions.https.HttpsError(
          'not-found',
          'Aucune liaison existante ou passée n\'a été trouvée pour cet utilisateur et ce membre du personnel.',
          { businessCode: 'STAFF_USER_LINK_NOT_FOUND' }
        );
      }
      ...
      if (!userPointerExists || !staffPointerExists) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Un seul des deux documents de pointeur existe en base.',
          { businessCode: 'LINK_INTEGRITY_ERROR' }
        );
      }
      ...
      if (
        userPointer.linkId !== staffPointer.linkId ||
        ...
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Incohérence d\'intégrité détectée sur les pointeurs.',
          { businessCode: 'LINK_INTEGRITY_ERROR' }
        );
      }
      ...
      if (!linkSnap.exists) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Le document historique de liaison correspondant est manquant.',
          { businessCode: 'LINK_INTEGRITY_ERROR' }
        );
      }
      ...
      if (
        linkDocData.userId !== cleanUserId ||
        linkDocData.staffId !== cleanStaffId ||
        linkDocData.schoolId !== cleanSchoolId
      ) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Incohérence d\'intégrité entre l\'historique et les pointeurs.',
          { businessCode: 'LINK_INTEGRITY_ERROR' }
        );
      }
      ...
      if (userPointer.isActive === false && staffPointer.isActive === false) {
        if (linkDocData.isActive !== false) {
          throw new functions.https.HttpsError(
            'failed-precondition',
            'Incohérence d\'état entre les pointeurs inactifs et l\'historique actif.',
            { businessCode: 'LINK_INTEGRITY_ERROR' }
          );
        }
      ...
      if (userPointer.isActive === false || staffPointer.isActive === false) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Divergence d\'état actif/inactif entre les deux pointeurs.',
          { businessCode: 'LINK_INTEGRITY_ERROR' }
        );
      }
      ...
      if (linkDocData.isActive !== true) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Incohérence d\'état: l\'historique est inactif alors que les pointeurs sont actifs.',
          { businessCode: 'LINK_INTEGRITY_ERROR' }
        );
      }

      // Logical dissociation: Update all three documents (do not delete)
      const historicUpdate: Record<string, unknown> = {
        isActive: false,
        updatedAt: nowIso,
        updatedBy: uid,
