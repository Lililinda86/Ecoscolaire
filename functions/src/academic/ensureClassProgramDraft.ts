  ...
  try {
    return await db.runTransaction(async (transaction) => {
      ...
      if (!isPublishedWithoutDraft) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Le programme présente une révision incohérente.',
          { businessCode: 'PROGRAM_INTEGRITY_ERROR' }
        );
      }
      ...
      if (!targetSubjectsSnap.empty) {
        throw new functions.https.HttpsError(
          'aborted',
          'La révision cible existe déjà.',
          { businessCode: 'REVISION_CONFLICT' }
        );
      }
      ...
      const publishedSubjects = publishedSubjectsSnap.docs.map(docSnap => {
        ...
        if (
          ...
          throw new functions.https.HttpsError(
            'failed-precondition',
            'Incohérence de données détectée dans les matières publiées.',
            { businessCode: 'PROGRAM_INTEGRITY_ERROR' }
          );
        }
        if (seenSubjectIds.has(data.subjectId)) {
          throw new functions.https.HttpsError(
            'failed-precondition',
            'Matière dupliquée détectée dans la révision publiée.',
            { businessCode: 'PROGRAM_INTEGRITY_ERROR' }
          );
        }
      ...
      if (publishedSubjects.length > MAX_CLONED_SUBJECTS) {
        throw new functions.https.HttpsError(
          'resource-exhausted',
          'Le programme contient trop de matières à cloner.',
          { businessCode: 'PROGRAM_TOO_LARGE' }
        );
      }
