...
exports.seedDefaultSubjectCatalog = void 0;
exports.normalizeSubjectText = normalizeSubjectText;
const functions = require("firebase-functions");
const admin = require("firebase-admin");
const defaultSubjectCatalog_1 = require("./defaultSubjectCatalog");
...
exports.seedDefaultSubjectCatalog = functions.https.onCall(async (data, context) => {
    ...
    try {
        return await db.runTransaction(async (transaction) => {
            ...
            if (!allowedRoles.includes(operator.role)) {
                throw new functions.https.HttpsError('permission-denied', 'Rôle non autorisé à gérer le catalogue.', { businessCode: 'PERMISSION_DENIED' });
            }
            if (operator.role !== 'superAdmin' && operator.schoolId !== cleanSchoolId) {
                throw new functions.https.HttpsError('permission-denied', 'L\'opérateur n\'appartient pas à l\'école demandée.', { businessCode: 'SCHOOL_MISMATCH' });
            }
            ...
            const detSnapsMap = new Map();
            for (const candidate of defaultSubjectCatalog_1.DEFAULT_SUBJECT_CATALOG) {
            ...
            for (const candidate of defaultSubjectCatalog_1.DEFAULT_SUBJECT_CATALOG) {
                ...
                if (detSnap.exists) {
                    ...
                    if (sameSection && hasCommonCycle && (normExistingName === normCandidateName || (candidate.aliases || []).map(normalizeSubjectText).includes(normExistingName))) {
                        ...
                        throw new functions.https.HttpsError('failed-precondition', `Une matière existante utilise l'identifiant ${detId} avec des informations incompatibles.`, { businessCode: 'SUBJECT_SEED_CONFLICT' });
                    }
                }
                ...
                for (const existing of existingSubjects) {
                    ...
                    if (sameCode) {
                        ...
                        if (sameSection && hasCommonCycle && isNameOrAliasMatch) {
                            ...
                            throw new functions.https.HttpsError('failed-precondition', `Une matière existante utilise le code ${candidate.code} avec des informations incompatibles.`, { businessCode: 'SUBJECT_SEED_CONFLICT' });
                        }
                    }
                    const sameSection = existing.section === candidate.section;
                    const hasCommonCycle = candidate.cycles.some((c) => (existing.cycles || []).includes(c));
            ...
            return {
                ...
                totalCandidates: defaultSubjectCatalog_1.DEFAULT_SUBJECT_CATALOG.length,
                createdCount,
                skippedCount,
                existingByCodeCount,
                existingByAliasCount,
...
//# sourceMappingURL=seedDefaultSubjectCatalog.js.map